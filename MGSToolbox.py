...
GLOBAL_VARS = {
    ...
    "Script Locations": {
        "GATHER_DATA_SCRIPT": "Church Registers/gather_data.py",
        "CHURCH_GEDCOM_SCRIPT": "Church Registers/create_gedcom.py",
        "CENSUS_GEDCOM_SCRIPT": "1850-1870 US Census/convert_census_1850-1870.py",
        "DUPE_SCRIPT": "Dupes/Dupes.py",
        "COUNTY_SCRIPT": "County Fix/CountyFix.py",
        "CLEANUP_CACHE_SCRIPT": "Church Registers/cleanup_cache.py"
    },
    "Global Directories": {
        "PROGRAM_DIR": "C:/Path/To/Your/Genealogy/Folder",
        "RM_DIR": "Roots Magic 11",
        "FTM_DIR": "Family Tree Maker",
        "CSV_DIR": "CSV/Project",
        "JSON_DIR": "Church Registers/JSON",
        "IMAGE_EXTENSION": "jpg",
        "GEDCOM_OUTPUT_PATH": "RootsMagic 11/Gedcom/Project"
    },
    "Metadata & Organization": {
        "RESEARCHER": "Your Name",
        "ORG_NAME": "Your Historical Society",
...
CHURCH_VARS = {
    "Data & Directories": {
        "CHURCH_IMAGE_DIR": "RootsMagic 11/Media/Project/Parish",
    ...
    "Register Information": {
        ...
        "VOLUME_TITLE": "Baptisms, marriages and burials, 1850-1900",
        "VOLUME_NUM": "1",
    ...
    "Formatting & Roles": {
        "TRANSCRIPTION_HEADER": "Original Transcription:",
...
CENSUS_VARS = {
    "Data & Directories": {
        "CENSUS_IMAGE_DIR": "RootsMagic 11/Media/Project/1850 US Federal Census",
    ...
    "Location & Schedule": {
        ...
        "FILM_NUMBER": "94",
        "ROLL_NUMBER": "M653",
        "CENSUS_PUBLISHER": "Publisher, Inc.",
        "CENSUS_PUB_LOC": "Anytown, ST, USA"
    },
    "Census Citation (Source)": {
        "CENSUS_CALL_NUMBER": "4195937",
        "CENSUS_REPOSITORY": "NARA",
    ...
    "Ancestry Information": {
        "ANCESTRY_START_RECORD_ID": "1000000",
        "APID_DB": "7667",
...
DUPE_VARS = {
    "File Paths (Relative to Program Dir)": {
        "DUPE_RM_DATABASE": "RootsMagic 11/Your Tree.rmtree"
    },
    "Matching Thresholds": {
        "DUPE_FUZZY_THRESHOLD": "82",
        "DUPE_MAX_AGE_GAP": "5",
        "DUPE_FUZZY_THRESHOLD_STRICT": "95",
        "DUPE_FAMILY_MATCH_THRESHOLD": "75"
    },
    "RootsMagic UI Settings": {
        "DUPE_FOLDER_NAME": "!Duplicate Review",
        "DUPE_COLOR_SET": "1",
        "DUPE_COLOR_VALUE": "27"
    }
}
...
COUNTY_VARS = {
    "File Paths (Relative to Program Dir)": {
        "COUNTY_RM_DATABASE": "RootsMagic 11/Your Tree.rmtree",
        "COUNTY_SHAPEFILE": "County Fix/US_AtlasHCB_Counties/US_HistCounties_Shapefile/US_HistCounties.shp"
    },
    "Settings": {
        "COUNTY_DEBUG_MODE": "False",
        "COUNTY_CREATE_BACKUP": "True"
    }
}
...
class ModularToolbox(ctk.CTk):
    ...
    def _build_layout(self):
        ...
        ctk.CTkLabel(self.sidebar, text="MGS Toolbox", font=ctk.CTkFont(size=22, weight="bold")).grid(row=0, column=0, padx=20, pady=(20, 10))

        nav_buttons = [
            ("Church Register", "church"),
            ("Census Data", "census"),
            ("Duplicate Finder", "dupe"),
            ("County Fixer", "county"),
        ...
        # --- LOCKED STATUS BAR ---
        self.status_bar = ctk.CTkEntry(self.console_frame, font=ctk.CTkFont(family="Consolas", size=13, weight="bold"), text_color="#00FFFF", fg_color="#1a1a1a", border_width=1)
        ...
        self.console_text = ctk.CTkTextbox(self.console_frame, font=ctk.CTkFont(family="Consolas", size=12), text_color="#00FF00")
        ...
        self.console_input = ctk.CTkEntry(self.input_frame, placeholder_text="Type script input here and press Enter...", state="disabled")
    ...
    def _build_form_ui(self, parent, schema_dict):
        ...
        for section, fields in schema_dict.items():
            ctk.CTkLabel(scroll, text=section, font=ctk.CTkFont(size=16, weight="bold"), text_color="#3B8ED0").pack(anchor="w", pady=(15, 5))
    ...
    def _build_tabs(self):
        ...
        ctk.CTkLabel(frame_global, text="Global Environment Settings", font=ctk.CTkFont(size=24, weight="bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(frame_church, text="Church Register Workflow", font=ctk.CTkFont(size=24, weight="bold")).pack(anchor="w")
        ...
        ctk.CTkLabel(debug_frame, text="Debug Image Filename (Leave blank for Batch):", font=ctk.CTkFont(weight="bold")).pack(side="left")
        ...
        ctk.CTkButton(btn_box, text="Step 1: Gather Data (API)", fg_color="darkred", hover_color="red", command=lambda: self.execute_script("GATHER_DATA_SCRIPT", "church_api")).pack(side="left", padx=5)
        ctk.CTkButton(btn_box, text="Step 2: Generate GEDCOM", fg_color="darkgreen", hover_color="green", command=lambda: self.execute_script("CHURCH_GEDCOM_SCRIPT", "church_gedcom")).pack(side="left", padx=5)
        ctk.CTkButton(btn_box, text="Clear Cache", fg_color="#B85D19", hover_color="#8A4513", command=lambda: self.execute_script("CLEANUP_CACHE_SCRIPT", "standalone")).pack(side="left", padx=5)

        # 3. Census Data
        frame_census = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.frames["Census Data"] = frame_census
        ctk.CTkLabel(frame_census, text="Census Data Workflow", font=ctk.CTkFont(size=24, weight="bold")).pack(anchor="w")
        ...
        ctk.CTkButton(btn_box2, text="Generate Census GEDCOM", fg_color="darkgreen", hover_color="green", command=lambda: self.execute_script("CENSUS_GEDCOM_SCRIPT", "census")).pack(side="left", padx=5)

        # 4. Duplicate Finder
        frame_dupe = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.frames["Duplicate Finder"] = frame_dupe
        ctk.CTkLabel(frame_dupe, text="Duplicate Finder Workflow", font=ctk.CTkFont(size=24, weight="bold")).pack(anchor="w")
        ctk.CTkLabel(frame_dupe, text="Finds logical duplicate people in RootsMagic.", text_color="gray").pack(anchor="w", pady=(0, 20))
        ...
        ctk.CTkButton(btn_box3, text="Run Script", fg_color="purple", hover_color="darkviolet", command=lambda: self.execute_script("DUPE_SCRIPT", "standalone")).pack(side="left", padx=5)

        # 5. County Fixer
        frame_county = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.frames["County Fixer"] = frame_county
        ctk.CTkLabel(frame_county, text="County Fixer Workflow", font=ctk.CTkFont(size=24, weight="bold")).pack(anchor="w")
        ctk.CTkLabel(frame_county, text="Fixes historical US county jurisdictions utilizing Geopandas.", text_color="gray").pack(anchor="w", pady=(0, 20))
        ...
        ctk.CTkButton(btn_box4, text="Run Script", fg_color="purple", hover_color="darkviolet", command=lambda: self.execute_script("COUNTY_SCRIPT", "standalone")).pack(side="left", padx=5)

    def execute_script(self, script_key, mode):
        """Prepares environment variables and launches a script in a background thread."""
        script_path = self.string_vars.get(script_key)
        ...
        program_dir = self.string_vars.get("PROGRAM_DIR", ctk.StringVar()).get().strip()
        ...
        if mode.startswith("church"):
            dynamic_keys = {
                "IMAGE_DIR": self.string_vars["CHURCH_IMAGE_DIR"].get(),
                ...
                "CALL_NUMBER": self.string_vars.get("CHURCH_CALL_NUMBER", ctk.StringVar()).get(),
                "REPOSITORY": self.string_vars.get("CHURCH_REPOSITORY", ctk.StringVar()).get(),
                "REPOSITORY_LOC": self.string_vars.get("CHURCH_REPOSITORY_LOC", ctk.StringVar()).get(),
                "COLLECTION_URL": self.string_vars.get("CHURCH_COLLECTION_URL", ctk.StringVar()).get(),
                "COLLECTION_NAME": self.string_vars.get("CHURCH_COLLECTION_NAME", ctk.StringVar()).get()
            }
        elif mode == "census":
            dynamic_keys = {
                "IMAGE_DIR": self.string_vars["CENSUS_IMAGE_DIR"].get(),
                ...
                "CALL_NUMBER": self.string_vars.get("CENSUS_CALL_NUMBER", ctk.StringVar()).get(),
                "REPOSITORY": self.string_vars.get("CENSUS_REPOSITORY", ctk.StringVar()).get(),
                "REPOSITORY_LOC": self.string_vars.get("CENSUS_REPOSITORY_LOC", ctk.StringVar()).get(),
                "COLLECTION_URL": self.string_vars.get("CENSUS_COLLECTION_URL", ctk.StringVar()).get(),
                "COLLECTION_NAME": self.string_vars.get("CENSUS_COLLECTION_NAME", ctk.StringVar()).get()
            }
